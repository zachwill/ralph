#!/usr/bin/env bun
/**
 * Example: ralph with a simple supervisor (prompt-only; no custom logic).
 */
import { generate, loop, supervisor, work } from "../core";

const LOOP_NAME = "ralph-simple-supervisor";
...
loop({
  ...
  taskFile: TASK_FILE,
  timeout: TIMEOUT,
  supervisor: supervisor(supervisorPrompt, { every: 12, model: "claude-opus-4-5" }),

  run(state) {
    return state.hasTodos ? work(workerPrompt) : generate(plannerPrompt);
  },
});
